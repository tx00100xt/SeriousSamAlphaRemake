...
properties:
  ...
  6 FLOAT m_vXprojectile = 1.0f,
  7 FLOAT m_vYprojectile = 1.5f,
...
// ************** SOUNDS ************** PlaySound(m_soSound, SOUND_BLOWUP, SOF_3D);
 ...
 58 sound   SOUND_BLOWUP    "Sounds\\Weapons\\_BulletHitFlesh.wav",
 ...
 ************************************************************/
  ...
  Fire(EVoid) : CEnemyBase::Fire { //FlyAttackEnemy(EVoid) { 
    if (!m_bInAir) {
      // wait for a while
      StopMoving();
      StartModelAnim(EYEMAN_ANIM_ATTACK01, 0);
      autowait(GetModelObject()->GetAnimLength(EYEMAN_ANIM_ATTACK01)-0.5f); // -0.5f for synhronisations with projecrile

      switch(m_EecChar) {
        case EYC_SERGEANT: { m_vXprojectile = 1.5f; m_vYprojectile = 2.0f;} break;
        case EYC_SOLDIER : { m_vXprojectile = 1.0f; m_vYprojectile = 1.5f;} break;
      }
      // fire projectile
      ShootProjectile(PRT_ACID, FLOAT3D( m_vXprojectile, m_vYprojectile, 0.5f), ANGLE3D(0, 0, 0));
