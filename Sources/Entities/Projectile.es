...
enum ProjectileType {
 ...
 30 PRT_LAVAMAN_STONE_BIG     "Lavaman Big Stone",   // lavaman rock projectile
 31 PRT_LAVAMAN_STONE_LARGE    "Lavaman Large Stone",   // lavaman rock projectile
 24 PRT_LAVAMAN_FIREBALL      "Lavaman Ball",   // lavaman Ball projectile
 25 PRT_LAVAMAN_FIREBALL_BIG  "Lavaman Big Ball",   // lavaman Ball projectile
 26 PRT_LAVAMAN_FIREBALL_LARGE "Lavaman Large Ball",   // lavaman Ball projectile
 ...
 70 PRT_DALPHA_PROJECTILE      "DevilAlpha Projectile",   // DevilAlpha projectile
 71 PRT_DALPHA_DEBRIS          "DevilAlpha Debris",   // DevilAlpha projectile's debris
};
...
void CProjectile_OnPrecache(CDLLEntityClass *pdec, INDEX iUser) 
{
  ...
  case PRT_LAVAMAN_STONE_BIG:
  case PRT_LAVAMAN_STONE_LARGE:
  case PRT_LAVAMAN_FIREBALL:
  case PRT_LAVAMAN_FIREBALL_BIG:
  case PRT_LAVAMAN_FIREBALL_LARGE:
    pdec->PrecacheModel(MODEL_ELEM_LAVA_STONE);
    pdec->PrecacheModel(MODEL_ELEM_LAVA_STONE_FLARE);
    pdec->PrecacheModel(MODEL_ELEM_LAVA_FIREBALL);
    ...
    pdec->PrecacheTexture(TEXTURE_ELEM_LAVA_FIREBALL); 
  ...
  case PRT_LAVA_COMET:
    ...
    pdec->PrecacheTexture(TEXTURE_LAVA_FLARE);
    break;
  case PRT_DALPHA_PROJECTILE:
  case PRT_DALPHA_DEBRIS:
    pdec->PrecacheSound(SOUND_DALPHA_FLYING  );
    pdec->PrecacheModel(MODEL_DALPHA_FIRE);
    pdec->PrecacheTexture(TEXTURE_DALPHA_FIRE);
...
// ********* PLAYER ROCKET *********
 ...
 13 sound   SOUND_DALPHA_FLYING  "Sounds\\Weapons\\ProjectileFly.wav",
...
// ********* PLAYER LASER *********
 ...
 51 model   MODEL_GLOW           "Models\\Weapons\\Laser\\Projectile\\LaserProjectileGlow01.mdl",
 52 texture TEXTURE_GREEN_LASER   "Models\\Weapons\\Laser\\Projectile\\LaserProjectile.tex",
 53 texture TEXTURE_BLUE_LASER    "Models\\Weapons\\Laser\\Projectile\\LaserProjectileBlue.tex",
...
// ********* ELEMENTAL FIRE *********
 ...
 87 model   MODEL_ELEM_LAVA_FIREBALL       "Models\\Enemies\\Elementals\\Projectile\\Fireball.mdl",
 ...
 97 texture TEXTURE_ELEM_LAVA_FIREBALL       "Models\\Enemies\\Elementals\\Projectile\\Fireball.tex",
...
// ********* DALPHA FIRE *********
170 model   MODEL_DALPHA_FIRE       "Models\\Enemies\\DevilAlpha\\Projectile\\Projectile.mdl",
171 texture TEXTURE_DALPHA_FIRE     "Models\\Enemies\\DevilAlpha\\Projectile\\Projectile.tex",
...
functions:
  ...
  void RenderParticles(void) {
    switch (m_prtType) {
      ...
      case PRT_DALPHA_PROJECTILE: Particles_DalphaProjectileTrail( this, 2.0f, 0.25f, 48); break;
      ...
      case PRT_DEVIL_GUIDED_PROJECTILE:
        ...
        break;
      case PRT_DALPHA_DEBRIS: Particles_DalphaProjectileDebrisTrail(this, 0.20f); break;
...
void PlayerLaserRay(void) {
  ...
  AddAttachmentToModel(this, *GetModelObject(), LASERPROJECTILE_ATTACHMENT_GLOW,
                       MODEL_GLOW, TEXTURE_GREEN_LASER, 0, 0, 0);
  CModelObject *pmo = GetModelObject();
  if(pmo != NULL)
  {
...
void LavaStone(void) {
  ...
  AddAttachmentToModel(this, *GetModelObject(), LAVABOMB_ATTACHMENT_FLARE,
                     MODEL_ELEM_LAVA_BOMB_FLARE, TEXTURE_ELEM_LAVA_BOMB_FLARE, 0, 0, 0); // Add flare to LRG
...
void ElementalRockFireBall(INDEX iSize) {
  // set appearance
  InitAsModel();
  SetPhysicsFlags(EPF_ONBLOCK_SLIDE|EPF_PUSHABLE|EPF_MOVABLE);
  SetCollisionFlags(ECF_PROJECTILE_SOLID);

  SetModel(MODEL_ELEM_LAVA_FIREBALL);
  SetModelMainTexture(TEXTURE_ELEM_LAVA_FIREBALL);
  GetModelObject()->StretchModel(FLOAT3D(2.25f, 2.25f, 2.25f));
  ModelChangeNotify();
  // start moving
  if (iSize==ELEMENTAL_LARGE) {
...
void LavamanBombDebrisExplosionLRG(void)
{
  ESpawnEffect ese;
  FLOAT3D vPoint;
  FLOATplane3D vPlaneNormal;
  ...
  ese.colMuliplier = C_WHITE|CT_OPAQUE;
  ese.betType = BET_LIGHT_CANNON;
  ese.vStretch = FLOAT3D(0.1f,0.1f,0.1f);
  SpawnEffect(GetPlacement(), ese);
...
void DalphaProjectile(void) {
  ...
  SetModel(MODEL_DALPHA_FIRE);
  SetModelMainTexture(TEXTURE_DALPHA_FIRE);
  GetModelObject()->StretchModel(FLOAT3D(1.5f, 1.5f, 1.5f));

  ModelChangeNotify();
  // play the flying sound
  m_soEffect.Set3DParameters(20.0f, 2.0f, 1.0f, 1.0f);
  PlaySound(m_soEffect, SOUND_DALPHA_FLYING, SOF_3D|SOF_LOOP);
  // start moving
...
void DalphaDebrisExplosion(void)
{
  // explosion
  ESpawnEffect ese;
  ...
  eSpawnSpray.sptType = SPT_DALPHA_PROJECTILE_SPRAY;
  eSpawnSpray.vDirection = en_vCurrentTranslationAbsolute/64.0f;
  eSpawnSpray.penOwner = this;
  penSpray->Initialize( eSpawnSpray);
};

void DalphaProjectileExplosion(void)
{
  // explosion
  ESpawnEffect ese;
  ese.colMuliplier = C_BLUE|CT_OPAQUE;
  ...
  for( INDEX iDebris=0; iDebris<2; iDebris++)
  {
    ...
    ELaunchProjectile eLaunch;
    eLaunch.penLauncher = this;
    eLaunch.prtType = PRT_DALPHA_DEBRIS;
    eLaunch.fSpeed = fSpeed;
    penProjectile->Initialize(eLaunch);
...
procedures:
  ...
  Main(ELaunchProjectile eLaunch) {
    ...
    switch (m_prtType) {
      ...
      case PRT_DALPHA_PROJECTILE: Particles_Fireball01Trail_Prepare(this); break;
    ...
    case PRT_WALKER_ROCKET: WalkerRocket(); break;
      ...
      case PRT_LAVAMAN_STONE_BIG: ElementalRock(ELEMENTAL_BIG, ELEMENTAL_LAVAMAN); break;
      case PRT_LAVAMAN_STONE_LARGE: ElementalRock(ELEMENTAL_LARGE, ELEMENTAL_LAVAMAN); break;
      case PRT_LAVAMAN_FIREBALL: ElementalRockFireBall(ELEMENTAL_NORMAL); break;
      case PRT_LAVAMAN_FIREBALL_BIG: ElementalRockFireBall(ELEMENTAL_BIG); break;
      case PRT_LAVAMAN_FIREBALL_LARGE: ElementalRockFireBall(ELEMENTAL_LARGE); break;
      ...
      case PRT_DALPHA_PROJECTILE: DalphaProjectile(); break;
      case PRT_DALPHA_DEBRIS: DalphaDebris(); break;
    ...
    switch (m_prtType) {
      ...
      case PRT_LAVASTONE: LavamanBombDebrisExplosionLRG(); break;
      //case PRT_LAVAMAN_STONE: LavamanBombDebrisExplosion(); break;
      //case PRT_LAVAMAN_STONE_BIG: LavamanBombDebrisExplosion(); break;
      //case PRT_LAVAMAN_STONE_LARGE: LavamanBombDebrisExplosion(); break;
      //case PRT_LAVAMAN_FIREBALL: LavamanBombDebrisExplosion(); break;
      //case PRT_LAVAMAN_FIREBALL_BIG: LavamanBombDebrisExplosion(); break;
      //case PRT_LAVAMAN_FIREBALL_LARGE: LavamanBombDebrisExplosion(); break;
      case PRT_DALPHA_PROJECTILE: DalphaProjectileExplosion(); break;
      case PRT_DALPHA_DEBRIS: DalphaDebrisExplosion(); break;   
