...
components:
  ...
  //3 class   CLASS_EPH        "Classes\\EnvironmentParticlesHolder.ecl",
...
//##############################################################
  void FixedAlphaIceMan(void)
  {
    //CPrintF("[FixedAlphaIceMan]\n");
    // for each entity in the world
    {FOREACHINDYNAMICCONTAINER(GetWorld()->wo_cenEntities, CEntity, iten) {
      CEntity *pen = iten;
      if (IsOfClass(pen, "Elemental")) {
        CElemental *penElemental = (CElemental *)pen;
        if (penElemental->m_EetType == ELT_ICE){
          penElemental->m_sptType = SPT_ICE;
        }
      } else if (IsOfClass(pen, "Mamut")) {
        CMamut *penMamut = (CMamut *)pen;
        penMamut->m_bFrontRider  = TRUE;
        penMamut->m_bMiddleRider = TRUE;
        penMamut->m_bRearRider   = TRUE;
      }
    }}
  }

  void FixedAlphaMamut(void)
  {
    //CPrintF("[FixedAlphaMamut]\n");
    // for each entity in the world
    {FOREACHINDYNAMICCONTAINER(GetWorld()->wo_cenEntities, CEntity, iten) {
      CEntity *pen = iten;
      if (IsOfClass(pen, "Mamut")) {
        CMamut *penMamut = (CMamut *)pen;
        penMamut->m_bFrontRider  = TRUE;
        penMamut->m_bMiddleRider = TRUE;
        penMamut->m_bRearRider   = TRUE;
  ...
  void FixedAlphaAirMan(void)
  {
    //CPrintF("[FixedAlphaAirMan]\n");
    // for each entity in the world
    {FOREACHINDYNAMICCONTAINER(GetWorld()->wo_cenEntities, CEntity, iten) {
      CEntity *pen = iten;
      if (IsOfClass(pen, "Elemental")) {
        CElemental *penElemental = (CElemental *)pen;
        if (penElemental->m_EetType == ELT_AIR){
          penElemental->m_sptType = SPT_TRAVA;
        }
      }
    }}
  }

  void InitializeDevilAlphaSirian(void)
  {
    //CPrintF("[Initialize DevilAlpha Sirian]\n");
    // for each entity in the world
    {FOREACHINDYNAMICCONTAINER(GetWorld()->wo_cenEntities, CEntity, iten) {
      CEntity *pen = iten;
      if (IsOfClass(pen, "DevilAlpha")) {
        CDevilAlpha *penDevilAlpha = (CDevilAlpha *)pen;
        INDEX iTYPE;
        iTYPE = (INDEX)penDevilAlpha->m_dacChar;
        penDevilAlpha->m_dacChar = DAC_SIRIUS;
        iTYPE = (INDEX) penDevilAlpha->m_dacChar;
      }
    }}
  }

  void InitializeDevilAlphaEgyptian(void)
  {
    //CPrintF("[Initialize DevilAlpha Egyptian]\n");
    // for each entity in the world
    {FOREACHINDYNAMICCONTAINER(GetWorld()->wo_cenEntities, CEntity, iten) {
      CEntity *pen = iten;
      if (IsOfClass(pen, "DevilAlpha")) {
        ...
        INDEX iTYPE;
        iTYPE = (INDEX)penDevilAlpha->m_dacChar;
        penDevilAlpha->m_dacChar = DAC_EGYPTIAN;
        iTYPE = (INDEX) penDevilAlpha->m_dacChar;
      }
    }}
  }

  void InitializeDevilAlphaPlanetoidal(void)
  {
    //CPrintF("[Initialize DevilAlpha Planetoidal]\n");
    // for each entity in the world
    {FOREACHINDYNAMICCONTAINER(GetWorld()->wo_cenEntities, CEntity, iten) {
      CEntity *pen = iten;
      if (IsOfClass(pen, "DevilAlpha")) {
        ...
        INDEX iTYPE;
        iTYPE = (INDEX)penDevilAlpha->m_dacChar;
        penDevilAlpha->m_dacChar = DAC_PLANETOIDAL;
        iTYPE = (INDEX) penDevilAlpha->m_dacChar;
      }
    }}
  }

  void FixedRobots(void)
  {
    // CPrintF("[Initialize DevilAlpha Planetoidal]\n");
    // for each entity in the world
    {FOREACHINDYNAMICCONTAINER(GetWorld()->wo_cenEntities, CEntity, iten) {
      CEntity *pen = iten;
      if (IsOfClass(pen, "Cyborg")) {
        CCyborg *penCyborg = (CCyborg *)pen;
        penCyborg->m_sptType = SPT_BOLTGAIKA;
      } else if (IsOfClass(pen, "RobotDriving")) {
        CRobotDriving *penRobotDriving = (CRobotDriving *)pen;
        penRobotDriving->m_sptType = SPT_BOLTGAIKA;
      } else if (IsOfClass(pen, "RobotFlying")) {
        CRobotFlying *penRobotFlying = (CRobotFlying *)pen;
        penRobotFlying->m_sptType = SPT_BOLTGAIKA;
      }
    }}
  }

  void FixedAlphaSpawners(INDEX imaxTotal, INDEX imaxGroupSized) {
    //CPrintF("[FixedAlphaSpawners]\n");
    // for each entity in the world
    {FOREACHINDYNAMICCONTAINER(GetWorld()->wo_cenEntities, CEntity, iten) {
      CEntity *pen = iten;
      if (IsOfClass(pen, "Enemy Spawner")) {
        CEnemySpawner *penCEnemySpawner = (CEnemySpawner *)pen;
        INDEX iTotal,iGroupSize;
        iTotal = (INDEX) penCEnemySpawner->m_ctTotal;
        iGroupSize = (INDEX) penCEnemySpawner->m_ctGroupSize;
        if (iTotal > imaxTotal) {
          penCEnemySpawner->m_ctTotal = imaxTotal;
        }
        if (iGroupSize > imaxGroupSized) {
          penCEnemySpawner->m_ctGroupSize = imaxGroupSized;
		}
  ...
  void FixedAlphaMovingBrash(void)
  {

    //CPrintF("[FixedAlphaMovingBrash(]\n");
    // for each entity in the world
    {FOREACHINDYNAMICCONTAINER(GetWorld()->wo_cenEntities, CEntity, iten) {
      CEntity *pen = iten;
      if (IsOfClass(pen, "Moving Brush")) {
        CMovingBrush *penCMovingBrush = (CMovingBrush *)pen;
        penCMovingBrush->m_bMoveOnTouch = TRUE;
        penCMovingBrush->m_bMoveOnDamage = TRUE;
      }
    }}
  }
/*
    ...
    {FOREACHINDYNAMICCONTAINER(GetWorld()->wo_cenEntities, CEntity, iten) {
      ...
      if (IsOfClass(pen, "EnvironmentParticlesHolder")) {
                CPrintF("[EnvironmentParticlesHolder pen->SendEvent(EStart()]\n");
...
//##############################################################
	...
	if ( strLevelName=="39_Vulcan") {
       InitializeDevilAlphaPlanetoidal();
    } else if ( strLevelName=="32_PyramidValley" ){
       InitializeDevilAlphaEgyptian();
    } else if ( strLevelName=="55_SpaceCrusader" ){
       InitializeDevilAlphaSirian();
    } else if ( strLevelName=="41_Ice" ) {
       FixedAlphaIceMan();
			//CreateSnow();
			//m_bStartSnow = TRUE;
    } else if ( strLevelName=="42_IceEnd" ) {
       FixedAlphaIceMan();
    } else if ( strLevelName=="43_TropicPlanet" ) {
       FixedAlphaAirMan();
    } else if ( strLevelName=="44_TropicEnd" ) {
       FixedAlphaAirMan();
    } else if ( strLevelName=="40_Green" ) {
       //FixedAlphaMamut();
    //} else if ( strLevelName=="11_AlleyOfSphinxes" ) {
	//		CreateSnow();
	//		m_bStartSnow = TRUE;
    }
    INDEX iLevel =  GetAlphaLevelNumber();
    //CPrintF("[Alpha Level Number: %d]\n", iLevel );
    if ( iLevel == 45 ) {
      //FixedAlphaSpawners(10,10);
    } else if ( iLevel > 46  && iLevel != 48) {
      FixedAlphaSpawners(5,5);
    } else if ( iLevel == 48 ) {
      FixedAlphaMovingBrash();
    }
	...
	if ( strLevelName=="GrandFinale1") {
       FixedAlphaSpawners(2,2);
    } else if ( strLevelName=="GrandFinale2" ){
       FixedAlphaSpawners(1,1);
       FixedAlphaMovingBrash();
