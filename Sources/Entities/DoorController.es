...
#ifdef PLATFORM_UNIX
extern "C" __attribute__ ((visibility("default"))) BOOL  _bPlayerShowVideoHint;
#else
DECL_DLL extern BOOL  _bPlayerShowVideoHint;
#endif
%}
        ...
        on (EPass ePass) : {
          if (CanReactOnEntity(ePass.penOther)) {
            if (m_strLockedMessage!="") {
              PrintCenterMessage(this, ePass.penOther, (const char *)TranslateConst((const char *)m_strLockedMessage), 3.0f, MSS_INFO);
            }
            if (m_penLockedTarget!=NULL) {
              _bPlayerShowVideoHint = TRUE;
        ...
        on (EPass ePass) : {
          if (IsDerivedFromClass(ePass.penOther, "Player")) {
            ...
            if (penPlayer->m_ulKeys&ulKey) {
              ...
              CTString strMsg;
              strMsg.PrintF(TRANS("%s used"), (const char *)GetKeyName(m_kitKey));
              PrintCenterMessage(this, ePass.penOther, (const char *)strMsg, 3.0f, MSS_INFO);
