...
static CTextureObject _toDalphaDebrisTrailGradient;
static CTextureObject _toDalphaDebrisTrailTexture;
static CTextureObject _toDalphaProjectileTrailTexture;
static CTextureObject _toDalphaProjectileTrailGradient;
static CTextureObject _toDalphaProjectileSprayTexture;

static CTextureObject _toBoltGaikaSprayTexture;
static CTextureObject _toPlasmaProjectileSpillTexture;
static CTextureObject _toTreeSpill01Texture;
...
void InitParticles(void)
{
  try
  {
    ...
    _toDalphaDebrisTrailGradient.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\DevilAlpha\\DalphaDebrisTrailGradient.tex"));
    _toDalphaProjectileTrailTexture.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\DevilAlpha\\DalphaProjectileTrail.tex"));
    _toDalphaProjectileTrailGradient.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\DevilAlpha\\DalphaProjectileTrailGradient.tex"));
    _toDalphaDebrisTrailTexture.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\DevilAlpha\\DalphaDebrisTrail.tex"));
    _toDalphaProjectileSprayTexture.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\DevilAlpha\\DalphaProjectileSpill.tex"));

    _toBoltGaikaSprayTexture.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\MetalSpill.tex"));
    _toPlasmaProjectileSpillTexture.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\PlasmaProjectileSpill.tex"));
    _toTreeSpill01Texture.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\TreeSpill01.tex"));
    ...
    ((CTextureData*)_toDalphaDebrisTrailGradient       .GetData())->Force(TEX_STATIC);
    ((CTextureData*)_toDalphaProjectileTrailGradient   .GetData())->Force(TEX_STATIC);
...
void CloseParticles(void)
{
  ...
  _toDalphaProjectileSprayTexture.SetData(NULL);
  _toDalphaDebrisTrailGradient.SetData(NULL);
  _toDalphaProjectileTrailTexture.SetData(NULL);
  _toDalphaProjectileTrailGradient.SetData(NULL);
  _toDalphaDebrisTrailTexture.SetData(NULL);

  _toBoltGaikaSprayTexture.SetData(NULL);
  _toPlasmaProjectileSpillTexture.SetData(NULL);
  _toTreeSpill01Texture.SetData(NULL);
}
...
#define DALPHA_PROJECTILE_DEBRIS_TRAIL_POSITIONS 8
#define DALPHA_PROJECTILE_DEBRIS_TRAIL_INTERPOSITIONS 1
void Particles_DalphaProjectileDebrisTrail_Prepare(CEntity *pen)
{
  pen->GetLastPositions(DALPHA_PROJECTILE_DEBRIS_TRAIL_POSITIONS);
}

void Particles_DalphaProjectileDebrisTrail(CEntity *pen, FLOAT fSizeMultiplier)
{
  CLastPositions *plp = pen->GetLastPositions(DALPHA_PROJECTILE_DEBRIS_TRAIL_POSITIONS);
  FLOAT fSeconds = _pTimer->GetLerpedCurrentTick();
  
  CTextureData *pTD = (CTextureData *) _toDalphaDebrisTrailGradient.GetData();
  Particle_PrepareTexture(&_toDalphaDebrisTrailTexture, PBT_BLEND);
  Particle_SetTexturePart( 512, 512, 0, 0);

  const FLOAT3D *pvPos1;
  const FLOAT3D *pvPos2 = &plp->GetPosition(0);
  INDEX iParticle = 0;
  INDEX iParticlesLiving = plp->lp_ctUsed*DALPHA_PROJECTILE_DEBRIS_TRAIL_INTERPOSITIONS;
  for(INDEX iPos = 1; iPos<plp->lp_ctUsed; iPos++)
  {
    pvPos1 = pvPos2;
    pvPos2 = &plp->GetPosition(iPos);
    for (INDEX iInter=0; iInter<DALPHA_PROJECTILE_DEBRIS_TRAIL_INTERPOSITIONS; iInter++)
    {
      FLOAT fAngle = iParticle*4.0f*180/iParticlesLiving;
      FLOAT3D vPos = Lerp(*pvPos1, *pvPos2, iInter*1.0f/DALPHA_PROJECTILE_DEBRIS_TRAIL_INTERPOSITIONS);
      FLOAT fRatio = FLOAT(iParticle)/iParticlesLiving+fSeconds;
...
#define DALPHA_PROJECTILE_TRAIL_POSITIONS 32
#define DALPHA_PROJECTILE_TRAIL_INTERPOSITIONS 1
void Particles_DalphaProjectileTrail_Prepare(CEntity *pen)
{
  pen->GetLastPositions(DALPHA_PROJECTILE_TRAIL_POSITIONS);
}

#define DALPHA_PROJECTILE_LINE_PARTICLES 0.4f
#define DALPHA_PROJECTILE_FADE_OUT 0.3f
#define DALPHA_PROJECTILE_TOTAL_TIME 0.6f
void Particles_DalphaProjectileTrail( CEntity *pen, FLOAT fSize, FLOAT fHeight, INDEX ctParticles)
{
  ASSERT( ctParticles<=CT_MAX_PARTICLES_TABLE);
  FLOAT fNow = _pTimer->GetLerpedCurrentTick();

  Particle_PrepareTexture(&_toDalphaProjectileTrailTexture, PBT_BLEND);
  Particle_SetTexturePart( 512, 2048, 0, 0);

  CTextureData *pTD = (CTextureData *) _toDalphaProjectileTrailGradient.GetData();
  ...
  for( INDEX iStar=0; iStar<ctParticles; iStar++)
  {
    ...
    // apply time strech
    fT *= 1/DALPHA_PROJECTILE_TOTAL_TIME;
    // get fraction part
    fT = fT-int(fT);
    //FLOAT fFade;
    //if (fT>(1.0f-DALPHA_PROJECTILE_FADE_OUT)) fFade=(1-fT)*(1/DALPHA_PROJECTILE_FADE_OUT);
    //else fFade=1.0f;
...
#define GET_POS( time) vCenter + \
    ...
    if( fT>DALPHA_PROJECTILE_LINE_PARTICLES)
    {
                          ...
                          FLOATaabbox3D boxOwner, FLOAT3D vSpilDirection, FLOAT tmStarted, FLOAT fDamagePower)
{
  ...
  switch(sptType) {
    ...
    case SPT_BOLTGAIKA:
    {
      Particle_PrepareTexture( &_toBoltGaikaSprayTexture, PBT_BLEND);
      break;
    }
    case SPT_BONES:
    {
    ...
    case SPT_PLASMA:
    {
      Particle_PrepareTexture( &_toPlasmaProjectileSpillTexture, PBT_BLEND);
      fDamagePower*=3.0f;
      break;
    }
    case SPT_TRAVA:
    {
      Particle_PrepareTexture( &_toTreeSpill01Texture, PBT_BLEND);
      fDamagePower*=3.0f;
      break;
    }
    ...
    case SPT_DALPHA_PROJECTILE_SPRAY:
    {
      Particle_PrepareTexture( &_toDalphaProjectileSprayTexture, PBT_BLEND);
      fDamagePower*=3.0f;
      break;
    }
