...
static CTextureObject _toBlueBar;
// ammo textures                    
...
extern struct WeaponInfo _awiWeapons[19];
static struct AmmoInfo _aaiAmmo[8] = {
  ...
  { &_toANapalm,      &_awiWeapons[11], NULL,             0, 0, 0, -9, FALSE },
  { &_toAElectricity, &_awiWeapons[13], &_awiWeapons[14], 0, 0, 0, -9, FALSE },
  { &_toAIronBall,    &_awiWeapons[16], NULL,             0, 0, 0, -9, FALSE },
  { &_toARocks,		  &_awiWeapons[17], NULL,             0, 0, 0, -9, FALSE },
};

struct WeaponInfo _awiWeapons[19] = {
...
extern void DrawHUD( const CPlayer *penPlayerCurrent, CDrawPort *pdpCurrent, BOOL bSnooping)
{
  ...
  // prepare and draw blue bar
  fRow = pixBottomBound;
  fCol = pixLeftBound;
  HUD_DrawIcon( fCol+350, fRow+2.1, _toBlueBar, C_dBLUE, fNormValue, FALSE);
...
*/
  ...
  //HUD_DrawBorder( fCol, fRow, fChrUnit*3, fOneUnit, colBorder);
  PrepareColorTransitions( colMax, colTop, colMid, C_RED, 0.4f, 0.25f, TRUE);
  _fdNumbersFont.SetCharSpacing(2.0);
  _fCustomScalingAdjustment = 0.5f;
  HUD_DrawText( fCol-5.4, fRow+5.5, strValue, NONE, fNormValue);
     PrepareColorTransitions( colTop, colMed, colMid, C_GREEN, 0.4f, 0.25f, TRUE);
  fCol -= fAdvUnit+fChrUnit*2/1.8 -fHalfUnit;
  HUD_DrawIcon( fCol-5.4, fRow+5.5, _toHealth, C_WHITE /*_colHUD*/, fNormValue, TRUE);
  ...
  if( fValue > 0.0f) {
    ...
    PrepareColorTransitions( colMax, colTop, colMid, C_lGRAY, 0.4f, 0.25f, TRUE);
...
*/
    ...
    _fCustomScalingAdjustment = 0.7f;
    //HUD_DrawBorder( fCol+fMoverX, fRow+fMoverY, fOneUnit, fOneUnit, colBorder);
    fCol += fAdvUnit+fChrUnit*1.8/2 -fHalfUnit;
    //HUD_DrawBorder( fCol, fRow, fChrUnit*3, fOneUnit, colBorder);
    _fCustomScalingAdjustment = 0.5f;
    HUD_DrawText( fCol-1.4, fRow+4.5, strValue, NONE, fNormValue);
    fCol -= fAdvUnit+fChrUnit*1.9/2 -fHalfUnit;
    HUD_DrawIcon( fCol-4.4, fRow+4.5, _toArmor, C_WHITE /*_colHUD*/, fNormValue, TRUE);
  }
  ...
  if( ptoCurrentAmmo!=NULL && !GetSP()->sp_bInfiniteAmmo) {
    ...
    //fCol += fAdvUnit+fChrUnit*3/2 -fHalfUnit;
    fCol += fAdvUnit+fChrUnit*12/2  -fHalfUnit;
    //HUD_DrawBorder( fCol, fRow, fChrUnit*3, fOneUnit, colBorder);
    if( bDrawAmmoIcon) {
      //fCol += fAdvUnit+fChrUnit*3/2 -fHalfUnit;
      ...
      //HUD_DrawBorder( fCol, fRow, fOneUnit, fOneUnit, colBorder);
      _fCustomScalingAdjustment = 0.5f;
      HUD_DrawIcon( fCol+5, fRow+5, *ptoCurrentAmmo, C_WHITE /*_colHUD*/, fNormValue, TRUE);
    ...
    HUD_DrawText( fCol-20.5, fRow+5, strValue, NONE, fNormValue);
    fCol -= fAdvUnit+fChrUnit*-4.5/2.1 -fHalfUnit; // To jest originalne 7/2.1
    HUD_DrawIcon( fCol+5, fRow+5, *ptoCurrentWeapon, C_WHITE /*_colHUD*/, fNormValue, !bDrawAmmoIcon);
    ...
    //HUD_DrawBorder( fCol, fRow, fOneUnit, fOneUnit, colBorder);
    _fCustomScalingAdjustment = 0.5f;
    HUD_DrawIcon(   fCol+6, fRow+6, *ptoCurrentWeapon, C_WHITE /*_colHUD*/, fNormValue, FALSE);
  }
...
*/
  ...
  if( _penPlayer->m_penMainMusicHolder!=NULL) {
    ...
    if (fNormValue>0) {
      ...
      PrepareColorTransitions( colMax, colMax, colMed, C_RED, 0.5f, 0.25f, FALSE);  
      fRow = pixTopBound + fOneUnit + fNextUnit;
      fCol = 184.0f;
...
//####
    ...
    bMaxScore  ? colScore  = C_WHITE : colScore  = C_WHITE;
    bMaxMana   ? colMana   = C_WHITE : colMana   = C_WHITE;
    bMaxFrags  ? colFrags  = C_WHITE : colFrags  = C_WHITE;
    bMaxDeaths ? colDeaths = C_WHITE : colDeaths = C_WHITE;
  }
  ...
  _pDP->SetFont( &_fdNumbersFont);
   pixCharWidth = _fdNumbersFont.GetWidth() + _fdNumbersFont.GetCharSpacing() +4;
  _pDP->SetTextScaling( fTextScale);
  _pDP->SetTextScaling(0.1f);
  ...
  HUD_DrawText(   fCol+fAdv, fRow, strValue, C_WHITE, fTextScale);
  HUD_DrawIcon(   fCol,      fRow, _toFrags, C_WHITE, 1.0f, TRUE);
...
extern void InitHUD(void)
{
  ...
  try {
	...
	_toBlueBar.SetData_t(  CTFILENAME("Textures\\General\\Bluebar.tex"));
	...
	((CTextureData*)_toBlueBar .GetData())->Force(TEX_CONSTANT);
