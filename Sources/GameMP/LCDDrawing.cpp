...
extern void _LCDRenderGrid(void)
{
  ...
  TiledTexture(_boxScreen, 0.35f, MEX2D(0,0),   boxBcgGrid);
  _pdp->PutTexture(&_toBcgGrid,   _boxScreen, boxBcgGrid, C_dGREEN|_ulA);
}

extern void LCDRenderConsoleBcg(void)
{
  MEXaabbox2D boxConsoleBcg;
  TiledTexture(_boxScreen, 0.9f*_pdp->GetWidth()/240.0f,
    MEX2D(1,25), boxConsoleBcg);
  _pdp->PutTexture(&_toConsoleBcg, _boxScreen, boxConsoleBcg);
 }
