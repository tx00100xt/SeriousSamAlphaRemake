  ...
  void SetCurrentWeaponModel(void) {
    ...
    switch (m_iCurrentWeapon) {
      ...
      case WEAPON_KNIFE:
		...
		m_moWeapon.StretchModel(FLOAT3D(0.6,0.6,0.6));
        break;
      // colt
      ...
      case WEAPON_SINGLESHOTGUN: {
		...
		m_moWeapon.StretchModel(FLOAT3D(0.75,0.75,0.75));
        break; }
      case WEAPON_DOUBLESHOTGUN: {
      ...
      case WEAPON_TOMMYGUN: {
		...
		m_moWeapon.StretchModel(FLOAT3D(0.845,0.845,0.845/*,0.87*/));
        break; }
      case WEAPON_MINIGUN: {
	  ...
	  case WEAPON_LRG:
		...
		m_moWeapon.StretchModel(FLOAT3D(0.61,0.61,0.61));
        break;
    }
  };
...
//      m_iNukeBalls+=eapi.iNukeBalls;
      ...
      if( eapi.iNapalm != 0) {strMessage.PrintF("%s %d %s,", (const char*)strMessage, eapi.iNapalm, TRANS("Napalm"));}
      if( eapi.iElectricity != 0) {strMessage.PrintF("%s %d %s,", (const char*)strMessage, eapi.iElectricity, TRANS("Batteries"));}
      if( eapi.iIronBalls != 0) {strMessage.PrintF("%s %d %s,", (const char*)strMessage, eapi.iIronBalls, TRANS("Cannonballs"));}
	  if( eapi.iRocks != 0) {strMessage.PrintF("%s %d %s,", (const char*)strMessage, eapi.iRocks, TRANS("Lava rocks"));}
...
//      case WEAPON_NUKECANNON:
  ...
  // Player Animator code(For jump start + end)
...
//   *  >>>---   WEAPON BORING   ---<<<
  ...
  FLOAT KnifeBoring(void) {
    ...
    INDEX iAnim;
    switch (m_iKnifeStand) {
      case 1: iAnim = KNIFE_ANIM_STAND1B; break;
      case 3: iAnim = KNIFE_ANIM_STAND3B; break;
      case 2: iAnim = KNIFE_ANIM_STAND2B; break;
    }
  ...
  FLOAT MiniGunBoring(void) {
    // play boring anim
    INDEX iAnim;
    switch (IRnd()%3) { // Remove 2,3,4 (3)
      case 0: iAnim = MINIGUN_ANIM_WAIT1; break; 
      case 1: iAnim = MINIGUN_ANIM_WAIT1; break;
      case 2: iAnim = MINIGUN_ANIM_WAIT1; break;
    }
    m_moWeapon.PlayAnim(iAnim, AOF_SMOOTHCHANGE);
    return m_moWeapon.GetAnimLength(iAnim);
  };
...
//      case WEAPON_NUKECANNON:
    ...
    // start animator
    CPlayerAnimator &plan = (CPlayerAnimator&)*((CPlayer&)*m_penPlayer).m_penAnimator;
    plan.BodyJumpStartAnimation();
	plan.BodyJumpEndAnimation();
...
/*
  ...
  Fire()
  {
    ...
    if (m_iCurrentWeapon==WEAPON_MINIGUN) {
      ...
      GetAnimator()->FireAnimation(BODY_ANIM_MINIGUN_STAND, AOF_LOOPING);
          ...
          wait() {
            on (EBegin) : {
              ...
              switch (m_iCurrentWeapon) {
                ...
                case WEAPON_ROCKETLAUNCHER: call NullSecondaryFire(); break;
                case WEAPON_GRENADELAUNCHER: call NullSecondaryFire(); break;
                case WEAPON_LASER: call NullSecondaryFire(); break;
                case WEAPON_LRG: call NullSecondaryFire(); break;
                case WEAPON_FLAMER: call NullSecondaryFire(); break;
                case WEAPON_PIPEBOMB: call NullSecondaryFire(); break;
                default: ASSERTALWAYS("Unknown weapon.");
			  }
              resume;
			}
...
/*
  ...
  FireSingleShotgun() {
    // fire one shell
    if (m_iShells>0) {
      GetAnimator()->FireAnimation(BODY_ANIM_SHOTGUN_STAND, 0);
  ...
  TommyGunStart() {
    ...
    GetAnimator()->FireAnimation(BODY_ANIM_SHOTGUN_STAND, AOF_LOOPING);
    return EEnd();
  };
  ...
  MiniGunSpinUp() {
	...
	m_moWeapon.PlayAnim(MINIGUN_ANIM_FIRESTART, AOF_NORESTART|AOF_NORESTART);
  ...
  MiniGunFire() {
    ...
    CPlayerAnimator &plan = (CPlayerAnimator&)*((CPlayer&)*m_penPlayer).m_penAnimator;
      GetAnimator()->FireAnimation(BODY_ANIM_MINIGUN_STAND, AOF_LOOPING);
	  m_moWeapon.PlayAnim(MINIGUN_ANIM_FIRELOOP, AOF_LOOPING|AOF_LOOPING);
    }
  ...
  MiniGunSpinDown() {
	...
	m_moWeapon.PlayAnim(MINIGUN_ANIM_FIRESTOP, AOF_NORESTART|AOF_NORESTART);
  ...
  FireRocketLauncher() {
    // fire one grenade
    if (m_iRockets>0) {
      GetAnimator()->FireAnimation(BODY_ANIM_MINIGUN_STAND, 0);
  ...
  FireGrenadeLauncher()
  {
    ...
    if (m_iGrenades>0)
    {
     // sound
      ...
      GetAnimator()->FireAnimation(BODY_ANIM_MINIGUN_STAND, 0);
	  m_moWeapon.PlayAnim(GRENADELAUNCHER_ANIM_FIRE, AOF_LOOPING|AOF_NORESTART);
	  //fire grenade
  ...
  // ***************** FIRE SecondaryDoubleShotgun *****************
  FireSecondaryDoubleShotgun()
  {
    //CPrintF("[FireSecondaryDoubleShotgun]\n");
    CPlacement3D plCrosshair;
    FLOAT fFX = wpn_fFX[m_iCurrentWeapon];  // get weapon firing position
    FLOAT fFY = wpn_fFY[m_iCurrentWeapon];
    if (GetPlayer()->m_iViewState == PVT_3RDPERSONVIEW) {
    ...
    if( crRay.cr_penHit!=NULL)
    {
      ...
      if( pen->GetFlags()&ENF_ALIVE)
	  {
		  ...
		  {
			  m_strLastTarget.PrintF(TRANS("[Healt this object] Life left is %d"),
...
// ***************** FIRE LAVA ROCKS *****************
  ...
  ChangeKnifeStand(EVoid) {
  	  CTString strValue;
	  strValue.PrintF("%d", (SLONG)m_iKnifeStand);
    if (m_iKnifeStand==1) {
      ...
      m_moWeapon.PlayAnim(KNIFE_ANIM_STAND2TOSTAND1, 0);
      autowait(m_moWeapon.GetAnimLength(KNIFE_ANIM_STAND2TOSTAND1));
      m_iKnifeStand = 1;
    }
    return EEnd();
  };
