...
DECL_DLL SLONG ctl_slPlayerControlsSize = sizeof(pctlCurrent);
...
void CPlayer_Precache(void)
{
  ...
  pdec->PrecacheSound(SOUND_BULLETHITFLESH     );
...
186 sound SOUND_BLOWUP			"Sounds\\Weapons\\_BlowUp.wav",
187 sound SOUND_BULLETHITFLESH  "Sounds\\Weapons\\_BulletHitFlesh.wav",
...
*/
    ...
    if (m_penCamera!=NULL && ((CCamera&)*m_penCamera).m_bWideScreen && _bPlayerShowVideoHint==FALSE) {
      pdp->MakeWideScreen(&dpCamera);
      pdpCamera = &dpCamera;
    } else if (m_penCamera!=NULL && _bPlayerShowVideoHint==TRUE) {
      dpCamera.InitCloned(pdp,0.03000, 0.03000, 0.45000, 0.45000 );
      pdpCamera = &dpCamera;
    }
...
/*
    ...
    //CPrintF("[dpCamera.InitCloned: %f, %f, %f, %f   ]\n", (1 - maxWidth) / 2, (1 - maxHeight) / 2, maxWidth, maxHeight);
*/
    ...
    if (bListen && _bPlayerShowVideoHint==FALSE) {
  ...
  void RenderGameView(CDrawPort *pdp, void *pvUserData)
  {
    ...
    if (bDualHead||m_penCamera==NULL||_bPlayerShowVideoHint==TRUE) {
...
//                                        -en_vGravityDir*(fDamageAmmount/15.0f));
	...
	PlaySound(m_soBody, SOUND_BULLETHITFLESH, SOF_3D);
...
//    CPrintF("---APPLY: %g\n", paOriginal.pa_aRotation(1));
    ...
    if (Abs(_pTimer->CurrentTick()-m_tmMessagePlay)<_pTimer->TickQuantum*2) {
      ...
      if (!m_bComputerInvoked && GetSP()->sp_bSinglePlayer) {
        PrintCenterMessage(this, this, 
          (const char *)TRANS("Press USE to read the message!"), 5.0f, MSS_NONE);
      }
    }
  ...
  void AliveActions(const CPlayerAction &pa) 
  {
    ...
    // if camera is active
    if (m_penCamera!=NULL && _bPlayerShowVideoHint==FALSE ) {
 ...
 ************************************************************/
  ...
  AutoPickItem(EVoid)
  {
    ...
    StartModelAnim(PLAYER_ANIM_DEFAULT_ANIMATION, 0);
  ...
  AutoFallDown(EVoid)
  {
    StartModelAnim(PLAYER_ANIM_DEATH01, 0);
    CModelObject &moBody = GetModelObject()->GetAttachmentModel(PLAYER_ATTACHMENT_TORSO)->amo_moModelObject;
    moBody.PlayAnim(BODY_ANIM_BODY_DEATH_NORMAL, 0);
  ...
  AutoAppear(EVoid)
  {
    ...
    CModelObject &moBody = GetModelObject()->GetAttachmentModel(PLAYER_ATTACHMENT_TORSO)->amo_moModelObject;
    moBody.PlayAnim(BODY_ANIM_FORMIPMODELS, AOF_SMOOTHCHANGE);
  ...
  LogoFireMinigun(EVoid) 
  {
    ...
    if( penBcgViewer != NULL)
    {
      ...
      pwsc->m_fShakeIntensityZ = 0.0f*0.0f;
...
/*
  ...
  AutoStoreWeapon(EVoid) 
  {
    ...
    plan.BodyAnimationTemplate(BODY_ANIM_WAIT, 
      BODY_ANIM_PUSHTOWALKKNIFE, BODY_ANIM_PUSHTOWALKCOLT, BODY_ANIM_PUSHTOWALKCOLT, BODY_ANIM_PUSHTOWALKCOLT, AOF_SMOOTHCHANGE);
    ((CPlayerWeapons&)*m_penWeapons).m_iCurrentWeapon = WEAPON_NONE;
  ...
  Main(EVoid evoid)
  {
    ...
    wait() {
      ...
      on (ECameraStart eStart) : {
        ...
        if (_bPlayerShowVideoHint==FALSE) {
           // stop player
           if (m_penActionMarker==NULL) {
             SetDesiredTranslation(FLOAT3D(0.0f, 0.0f, 0.0f));
             SetDesiredRotation(ANGLE3D(0.0f, 0.0f, 0.0f));
